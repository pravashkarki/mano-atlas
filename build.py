...
def toc_html() -> str:
    ...
    for slug, _f, num, en, ne, cat, group in PAGES:
        ...
        if group != last_group:
            g_en, g_ne = GROUPS[group]
            art = SPOT.get(group, "")
            out.append(f'<div class="toc-grp{" has-art" if art else ""}">{art}<h3><span class="en">{g_en}</span><span class="ne">{g_ne}</span></h3></div>')
            last_group = group
